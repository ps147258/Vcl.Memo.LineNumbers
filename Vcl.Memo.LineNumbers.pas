...
procedure UpdateMax(var Changed: Boolean; var OutValue: Integer; InValue: Integer); //inline;
begin
...
begin
  ...
  begin
//    TImage(FControl).Picture.Graphic.SetSize(FCanvasSize.Width, FCanvasSize.Height);
    Exit;
  end;
  ...
  begin
    FPicture.Graphic.SetSize(FCanvasSize.Width, FCanvasSize.Height);
    Exit;
  end;
...
begin
  ...
  try
    ...
    b := False;
    UpdateMax(b, FCanvasSize.cx, Base.Width);
    UpdateMax(b, FCanvasSize.cy, Base.Height);
    ...
    begin
      ...
      if b then
        if FControl is TImage then
          TImage(FControl).Picture.Graphic.SetSize(FCanvasSize.Width, FCanvasSize.Height + FGap.y);
    end;
...
//      Sync.Image.Canvas.TextOut(W - Sync.Image.Canvas.TextWidth(S), L, S);
//      Inc(L, H);
      Inc(N);
    end;
  finally
